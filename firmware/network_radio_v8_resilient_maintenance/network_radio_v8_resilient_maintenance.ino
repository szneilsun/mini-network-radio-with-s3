...
#define NETWORK_RADIO_VERSION "0.8.0-resilient-maintenance"
...
<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>网络收音机 V8</title><style>:root{color-scheme:dark}body{max-width:880px;margin:24px auto;padding:0 16px;font:16px system-ui;background:#101827;color:#e5e7eb}section,pre,.station{background:#172234;padding:14px;border-radius:10px;margin:14px 0}button,input,select{box-sizing:border-box;padding:9px;margin:4px;border:0;border-radius:6px}input,select{width:100%}button{background:#38bdf8;color:#062032;font-weight:700}.warn{background:#fbbf24}.danger{background:#fb7185}.station small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#b7c6da}.active{outline:2px solid #38bdf8}.state{font-size:1.1em;color:#67e8f9}pre{overflow:auto}.volume{width:calc(100% - 10px)}</style><h1>ESP32-S3 网络收音机</h1><p>V8 · 自动恢复与设备维护</p>
<section><h2>正在播放</h2><div id="now" class="state">读取中…</div><button onclick="post('/api/player/play')">▶ 播放</button><button class="danger" onclick="post('/api/player/stop')">■ 停止</button><label>音量：<b id="vol">--</b>/21</label><input id="volume" class="volume" type="range" min="0" max="21" oninput="q('#vol').textContent=this.value" onchange="post('/api/player/volume?value='+this.value)"></section>
...
void setup() {
  ...
  loadPlaylist(); importBuiltinStations(); loadSecurity();
