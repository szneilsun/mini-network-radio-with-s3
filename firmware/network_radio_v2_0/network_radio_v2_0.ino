/*
 * Network Radio 2.0.0 standalone Arduino sketch.
...
uint32_t playlistChecksum(uint8_t count, uint8_t selected,
...
constexpr char kUserHtmlV201[] PROGMEM = R"HTML(
<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>网络收音机</title><style>.cover{display:block;margin-inline:auto;object-position:center center}.station img{display:block;object-position:center center}</style><style>
:root{--bg:#656b6a;--accent:#f2a51a;--text:#fff;--muted:#e6e6e6}*{box-sizing:border-box}body{margin:0;background:#414847;color:var(--text);font:16px system-ui,-apple-system,"PingFang SC","Microsoft YaHei",sans-serif}.app{max-width:720px;min-height:100vh;margin:auto;padding:22px clamp(18px,5vw,40px) 42px;background:var(--bg)}a{color:#fff}.settings{float:right;font-size:27px;text-decoration:none}.hero{text-align:center;padding:42px 0 16px}.cover{width:min(50vw,245px);height:min(50vw,245px);border-radius:22px;object-fit:contain;background:#f7f7f7;box-shadow:0 8px 25px #0004}.cover-fallback{display:none;place-items:center;width:min(50vw,245px);height:min(50vw,245px);margin:auto;border-radius:22px;background:linear-gradient(145deg,#f5a623,#d47b13);font-size:76px;font-weight:800}.station-name{margin:22px 0 5px;font-size:clamp(25px,5vw,34px);font-weight:700}.state{color:var(--accent);font-size:17px}.controls{display:flex;align-items:center;justify-content:space-around;margin:18px auto 24px;max-width:520px}.controls button{border:0;background:transparent;color:#fff;font-size:37px;cursor:pointer}.controls .play{width:100px;height:100px;border-radius:50%;background:#fff;color:#555;font-size:45px;box-shadow:0 7px 22px #0003}.volume{display:flex;gap:12px;align-items:center;margin:18px 0 28px}input[type=range]{width:100%;accent-color:var(--accent)}.list-title{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #ffffff42}.list-title h2{font-size:18px}.count{color:var(--muted);font-size:14px}.station-list{border-bottom:1px solid #ffffff42}.station{display:flex;align-items:center;gap:16px;width:100%;min-height:82px;padding:10px 6px;border:0;border-top:1px solid #ffffff42;background:transparent;color:#fff;text-align:left;cursor:pointer}.station.active{border-left:4px solid var(--accent);padding-left:2px;background:#ffffff12}.station img,.station .fallback{width:58px;height:58px;flex:0 0 58px;border-radius:12px;object-fit:contain;background:#f7f7f7}.station .fallback{display:grid;place-items:center;background:linear-gradient(145deg,#f5a623,#d47b13);font-size:24px;font-weight:700}.station b{font-size:18px}.station small{display:block;margin-top:4px;color:var(--muted)}.notice{padding:24px 4px;color:var(--muted);text-align:center}@media(max-width:480px){.app{padding:18px 16px 32px}.cover,.cover-fallback{width:56vw;height:56vw}.controls .play{width:88px;height:88px}.controls button{font-size:31px}}</style></head>
<body><main class="app"><a class="settings" href="/admin" aria-label="管理设置">⚙</a><section class="hero"><img id="cover" class="cover" alt="当前电台台标"><div id="coverFallback" class="cover-fallback">R</div><div id="stationName" class="station-name">加载中…</div><div id="state" class="state">正在连接设备</div></section><nav class="controls" aria-label="播放控制"><button id="previous" aria-label="上一台">◀</button><button id="play" class="play" aria-label="播放或暂停">▶</button><button id="next" aria-label="下一台">▶</button></nav><div class="volume"><span>🔉</span><input id="volume" type="range" min="0" max="21" aria-label="音量"><span>🔊</span></div><div class="list-title"><h2>电台列表</h2><span id="count" class="count"></span></div><section id="stations" class="station-list"></section></main>
<script>
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh()});api('/api/user/theme').then(theme=>{document.documentElement.style.setProperty('--bg',theme.background);document.documentElement.style.setProperty('--accent',theme.accent);const t=textureStyles[theme.texture]||textureStyles.none;q('.app').style.backgroundImage=t[0];q('.app').style.backgroundSize=t[1]}).catch(()=>{});loadStations().then(refresh).catch(e=>{const n=document.createElement('div');n.className='notice';n.textContent=e.message;q('#stations').replaceChildren(n)});
</script></body></html>
)HTML";

constexpr char kAdminHtmlV201[] PROGMEM = R"HTML(
<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>网络收音机 2.0.0 管理</title><style>.station img,.station .fallback{justify-self:center;align-self:center}.station img{display:block;object-position:center center}</style><style>
:root{color-scheme:dark}body{max-width:900px;margin:24px auto;padding:0 16px;background:#101827;color:#e5e7eb;font:16px system-ui,-apple-system,"PingFang SC","Microsoft YaHei",sans-serif}section,pre,.station{margin:14px 0;padding:14px;border-radius:10px;background:#172234}button,input,select{box-sizing:border-box;margin:4px;padding:9px;border:0;border-radius:6px}input,select{width:100%}button{background:#38bdf8;color:#062032;font-weight:700;cursor:pointer}.warn{background:#fbbf24}.danger{background:#fb7185}.station{display:grid;grid-template-columns:54px 1fr;gap:10px;align-items:center}.station img,.fallback{width:48px;height:48px;grid-row:span 2;object-fit:contain;border-radius:8px;background:#fff}.fallback{display:grid;place-items:center;background:#e89c27;color:#fff;font-weight:700}.station small{overflow:hidden;color:#b7c6da;text-overflow:ellipsis;white-space:nowrap}.actions{grid-column:1/-1}.actions button{min-width:70px}.active{outline:2px solid #38bdf8}.player{display:grid;gap:10px}.transport{display:flex;justify-content:center;align-items:center;gap:clamp(24px,7vw,66px)}.transport button{display:grid;place-items:center;margin:0}.skip{width:70px;height:60px;border-radius:16px;background:#263449;color:#dce6f5;font-size:24px}.play{width:84px;height:84px;border-radius:50%;background:#f8fafc;color:#172234;font-size:32px}.theme{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.theme label{display:grid;gap:5px}.theme input,.theme select{margin:0}.theme input[type=color]{height:52px;padding:4px;background:#fff;color-scheme:light}.volume{width:100%;accent-color:#38bdf8}pre{overflow:auto;white-space:pre-wrap}a{color:#67e8f9}@media(max-width:560px){.theme{grid-template-columns:1fr}.actions{white-space:nowrap;overflow-x:auto}.actions button{min-width:auto}.station small{white-space:normal}}</style></head>
<body><h1>ESP32-S3 网络收音机</h1><p>版本 2.0.0　<a href="/">返回播放器</a></p>
<section class="player"><h2>正在播放</h2><div id="now">读取中…</div><div class="transport"><button id="previous" class="skip" aria-label="上一台">◀◀</button><button id="play" class="play" aria-label="播放或暂停">▶</button><button id="next" class="skip" aria-label="下一台">▶▶</button></div><label>音量 <b><span id="volumeText">--</span>/21</b><input id="volume" class="volume" type="range" min="0" max="21"></label></section>
<section><h2>用户页面外观</h2><div class="theme"><label>页面颜色<input id="background" type="color" value="#656b6a"></label><label>强调颜色<input id="accent" type="color" value="#f2a51a"></label><label>纹理<select id="texture"><option value="none">无纹理</option><option value="dots">圆点</option><option value="grid">网格</option><option value="diagonal">斜纹</option><option value="cloud">祥云</option><option value="lattice">回纹窗格</option><option value="waves">水波</option><option value="bamboo">竹影</option><option value="ricepaper">宣纸</option><option value="porcelain">青花</option></select></label></div><button id="saveTheme">保存页面外观</button></section>
...
void configureWebServerV8() {
  ...
  server.on("/", HTTP_GET, [] { server.send_P(200, "text/html; charset=utf-8", kUserHtmlV201); });
  server.on("/admin", HTTP_GET, [] { if (requireAdmin()) server.send_P(200, "text/html; charset=utf-8", kAdminHtmlV201); });
