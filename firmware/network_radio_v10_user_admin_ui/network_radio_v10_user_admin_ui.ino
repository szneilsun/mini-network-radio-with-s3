/* Network Radio V10: public player UI plus protected administration UI. */

#define NETWORK_RADIO_VERSION "0.10.1-user-admin-ui"
...
void onPlaylistSelectionV8(uint8_t) {
  ...
  startSelectedStationV8("station selected");
}
...
void handlePlayerStopV8() { if (requireAdmin()) { playbackEnabled = false; audio.stopSong(); playerRequested = false; setPlayerMessage("stopped by user"); addLog("player", "stopped by user"); handlePlayerStatus(); } }
...
void handleUserPlay() {
  playbackEnabled = true;
  if (!startSelectedStationV8("play requested from user page")) {
...
void handleUserPrevious() {
  ...
  selectedStation = selectedStation == 0 ? stationCount - 1 : selectedStation - 1;
  persistPlaylist();
  startSelectedStationV8("previous station from user page");
  handlePlayerStatus();
}
...
void handleUserNext() {
  ...
  selectedStation = (selectedStation + 1) % stationCount;
  persistPlaylist();
  startSelectedStationV8("next station from user page");
  handlePlayerStatus();
}
