...
sketch_dir="${package_root}/network_radio_v3_2_status_lcd"
...
project_name="network_radio_v3_2_status_lcd.ino"
...
cp "${build_dir}/${project_name}.bin" "${output_dir}/network-radio-v3.2.0-status-lcd-ota.bin"
"$mklittlefs" -c "${sketch_dir}/data" -b 4096 -p 256 -s "$partition_size" \
  "${output_dir}/network-radio-v3.2.0-status-lcd-littlefs.bin"

"$esptool" --chip esp32s3 merge-bin \
  -o "${output_dir}/network-radio-v3.2.0-status-lcd-full.bin" \
  ...
  0x620000 "${output_dir}/network-radio-v3.2.0-status-lcd-littlefs.bin"
...
if [[ ${1:-} == "--flash" ]]; then
  ...
  "$esptool" --chip esp32s3 --port "$2" --baud 460800 \
    ...
    0x0 "${output_dir}/network-radio-v3.2.0-status-lcd-full.bin"
elif [[ $# -ne 0 ]]; then
